...
def check_info(houstList, recordList):
    for record in recordList:
        ...
        msgTo = record.remind
        content = ""
        selectSql = "select id from remind where user_id = '{user_id}' and city = '{city}' and record_id = '{record_id}'"
        insertSql = "INSERT INTO remind (user_id, city, record_id) VALUES ('{user_id}', '{city}', '{record_id}') "
        for key in key_list:
            for houst in houstList:
                if key in houst.title or key in houst.content:
                    s_sql = selectSql.replace("{user_id}", record.open_id)
                    s_sql = s_sql.replace("{city}", record.city_name)
